...
def save_output(outputDir, history):
    ...
    # Pandas → torch.Tensor
    # output: ロジット想定（そのまま）
    outputs = torch.tensor(df["output"].to_numpy(), dtype=torch.float32).view(-1, 1)
    # target: 0/1 のラベル
    targets = torch.tensor(df["target"].to_numpy(), dtype=torch.float32).view(-1, 1)

    criterion = nn.BCEWithLogitsLoss()  # reduction="mean" がデフォ
    bce = criterion(outputs, targets)  # scalar tensor

    # 損失をファイル名に入れて保存
    df.to_csv(os.path.join(outputDir, f"{bce.item():.4f}.csv"), index=False)
    return
